%% Analysis of sliding door experiments
% This code was originally generated by H. Chiu and modified by C.E.
% Schretter for use with data reprocessed using the FlyDisco pipeline

% This code was used for mixed pair analysis in Figure 2 Figure Supplement
% 1

% Inputs are scores files which were scored using the classifier detailed
% in Chiu et al. 2025. 

% All videos were tracked using the Caltech FlyTracker and 
% subsequently manually fixed using FixErrors
% http://ctrax.sourceforge.net/fixerrors.html. 
% 
% Updated perframe features
% were generated using the FlyDisco Pipeline: https://github.com/kristinbranson/FlyDiscoAnalysis.

%% generate actions from scores
% this is for loading into Caltech FlyTracker Visualizer

clear all 
clear Cache
% Add JABBA to your path
addpath /Users/schretterc/Documents/GitHub/JAABA/perframe
% Add the folder in which the scores are stored 
addpath /Users/schretterc/Documents/
MainDir = uigetdir(pwd, 'Select a Folder');
FileList = dir(fullfile(MainDir,'*/movie.mp4'));

for j= 1:length(FileList)

    % enter each folder 
    Folder = FileList(j).folder(1:end);

    % creat a list of scores files (sturcture)
    ScoreList(1).name = 'scores_headbutt2_HC.mat';

    % assign fly1/fly2 scores of each behavior to the action file
    for i =1:length(ScoreList)
        cd([Folder])
        load(ScoreList(i).name);

        fly1_t0s = allScores.t0s{1,1}'; %start frame of behavioral bouts
        fly1_t1s = allScores.t1s{1,1}'; %end frame of behavioral bouts
        fly2_t0s = allScores.t0s{1,2}';
        fly2_t1s = allScores.t1s{1,2}';

        behs(1,i) = {behaviorName};
        bouts{1,i} = [fly1_t0s fly1_t1s 2*ones(length(fly1_t0s),1)];
        bouts{2,i}=[fly2_t0s fly2_t1s 2*ones(length(fly2_t0s),1)];

    end

    % save action file to the same layer as the movie file
    cd([MainDir])
    ActionName = [Folder '-actions.mat'];
    save(ActionName, 'behs', 'bouts')

end

% DestinationFolder = '/Volumes/KS_HCData/';
% FullFile = dir(fullfile(pwd, '*-actions.mat'));
% for file=1:length(FullFile)
% movefile(FullFile(file).name, DestinationFolder)
% end
% 
% cd(DestinationFolder)

%% Analysis for only saving and displaying the experimental genotype 
% The 'experimental genotype' here is the non-Canton S fly 

%For experiments in which Fly 1 is the experimental genotype
cd flipped/
ActionListB = dir([pwd '/' '*-actions.mat']);
MetaDataB = cell(length(ActionListB),1);

    for f=1:length(ActionListB)
        load(ActionListB(f).name)
        % fill null
        for fly=1:2
            for behavior=1:length(bouts)
                if isempty(bouts{fly, behavior})
                    bouts{fly, behavior}(1,1) = 0; %fly 1 is the experimental genotype
                    bouts{fly, behavior}(1,2) = 0; %fly 2 is the target
                end
            end
        end
        
    MetaDataB{f,1} = ActionListB(f).name(1:end);
    % headbutt - associate this with where the location of each of behavior
    % in the action file 
    % behav - seq of the behavior; its in one column; but frame rate is in
    % rows; 
    % headbutting only for fly 1
    MetaDataB{f,2} = bouts{1,1}(:,1);
    MetaDataB{f,2} = MetaDataB{f,2}(MetaDataB{f,2}>0);
    MetaDataB{f,3} = [bouts{1,1}(:,2)- bouts{1,1}(:,1)]; % duration
    MetaDataB{f,3} = MetaDataB{f,3}(MetaDataB{f,3}>0);
    end
cd ..
save('MetaData_flipped.mat', 'MetaDataB') 

%For experiments in which Fly 1 is the wild type fly
%append the .mat file 
cd regular/
ActionListC = dir([pwd '/' '*-actions.mat']);
MetaDataC = cell(length(ActionListC),1);

    for f=1:length(ActionListC)
        load(ActionListC(f).name)
        % fill null
        for fly=1:2
            for behavior=1:length(bouts)
                if isempty(bouts{fly, behavior})
                    bouts{fly, behavior}(1,1) = 0; %fly 1 is the target
                    bouts{fly, behavior}(1,2) = 0; %fly 2 is the experimental genotype
                end
            end
        end
        
    MetaDataC{f,1} = ActionListC(f).name(1:end);
    % headbutt - associate this with where the location of each of behavior
    % in the action file 
    % behav - seq of the behavior; its in one column; but frame rate is in
    % rows; 
    % headbutting only for fly 1
    MetaDataC{f,2} = bouts{2,1}(:,1);
    MetaDataC{f,2} = MetaDataC{f,2}(MetaDataC{f,2}>0);
    MetaDataC{f,3} = [bouts{2,1}(:,2)- bouts{2,1}(:,1)]; % duration
    MetaDataC{f,3} = MetaDataC{f,3}(MetaDataC{f,3}>0);
    end
cd ..
save('MetaData_regular.mat', 'MetaDataC')  

%combine flipped and regular Metadata 
a1=load('MetaData_flipped.mat')
f1=fieldnames(a1)
a2=load('MetaData_regular.mat')
f2=fieldnames(a2);
MetaData=[a1.(f1{1});a2.(f2{1})]
save('MetaData4.mat','MetaData')

%% raster

%parameters
npairs=length(MetaData);
Framerate = 30;
Time = 2.5*60*Framerate;

% color settings
 LED_color = [224/255 224/255 224/255]; % grey LED stimulation
 color_P1 = [239/255 86/255 117/255]; % #ef5675 pink headbutt
 color_B1 = [0/255 0/255 153/255];
 color_G1 = [0/255 128/255 0/255]; 
 
subplot(4,1,4)

    % plotting photostimulation period
    rectangle('Position',[30*30 .5 30*30 npairs+1],'FaceColor', LED_color,...
            'EdgeColor', 'none');
    hold on

    %headbutt 
    catTime = [];
    catDuration = [];
    catNumber = [];

    for g= 1:npairs
        catTime = vertcat(catTime, MetaData{g, 2});   % start 
        catDuration = vertcat(catDuration, MetaData{g, 3}); % duration 
        catNumber = vertcat(catNumber, g*ones(length(MetaData{g, 2}), 1)); % order                                             
    end
    for i=1:length(catNumber)
        rectangle('Position', [catTime(i) catNumber(i) ...         
            catDuration(i) 1], 'FaceColor', color_P1, ...
            'EdgeColor', color_P1); 
    end
 
    hold on
        
    %figure settings
    ax = gca;
    ax.Box = 'off';
    ax.LineWidth = 1;
    ax.XLim = [0 2.5*60*Framerate];
    ax.XTick = [0 .5*60*Framerate 1*60*Framerate 1.75*60*Framerate 2.5*60*Framerate];
    ax.TickLength = [0.003 0.003];
    ax.XTickLabel = {'0','.5','1','1.75','2.5'};
    ax.XLabel.String = 'Time (min)';
    ax.YLim = [-.1 npairs+2];
    ax.YTick = [1 npairs];
    ax.YTickLabel = [];
    ax.YLabel.String = 'Genotype';


